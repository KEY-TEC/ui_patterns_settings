 ...
 */
class PatternWithSettings extends RenderElement {
  ...
  public static function processMultipleSources(array $element) {
    ...
    if (self::hasFields($element) && self::hasMultipleSources($element)) {
      foreach ($element['#fields'] as $name => $field) {
        ...
        if (count($field) > 1) {
          /** @var \Drupal\ui_patterns_settings\Element\PatternContext $context */
          $context = $element['#context'];
   ...
   *   Render array.
   *
   * @throws \Drupal\ui_patterns_settings\Exception\PatternRenderException
